...
#        return fn_png
    ...
    def get_qr(self,sessionID, filename=None):
        """Get pairing QR code from client"""
        if "Click to reload QR code" in self.driver.page_source:
            self.reload_qr()
        qr = self.driver.find_element_by_css_selector(self._SELECTORS['qrCode'])
        ...
        #    fn_png = os.path.abspath(filename)
	print("qr")
	fd = os.open(self.username + '.png', os.O_RDWR | os.O_CREAT)
        fn_png = os.path.abspath(self.username + '.png')
        self.logger.debug("QRcode image saved at %s" % fn_png)	
	print (fn_png)
        qr.screenshot(fn_png)
	with open(fn_png, "rb") as imageFile:
    ...
    def get_unread(self, include_me=True, include_notifications=False, use_unread_count=False):
        """
